...
async function updateAppointment(res,url,headers,body){
  if(!body.id) return res.status(400).json({error:'Randevu kimliği eksik.'});
  ...
  if(scheduleChanged){
    ...
    if(Number.isNaN(day.getTime())||day.getDay()===0)return res.status(400).json({error:'Pazar günleri servis kapalıdır.'});
...
module.exports._test={allowedTimes,overlaps,clean};

...
function renderOverview(){
  ...
  renderStatusAnalytics();renderServiceBars();bindOpenButtons();
}
...
$('#clearFilters').addEventListener('click',()=>{$('#tableSearch').value='';$('#statusFilter').value='';$('#dateFilter').value='';$('#sortFilter').value='appointmentAsc';renderTable()});$('#exportCsv').addEventListener('click',exportCsv);
$('#scheduleDate').addEventListener('change',event=>{state.scheduleDate=event.target.value;renderSchedule()});$('#prevDay').addEventListener('click',()=>{state.scheduleDate=addDays(state.scheduleDate,-1);renderSchedule()});$('#nextDay').addEventListener('click',()=>{state.scheduleDate=addDays(state.scheduleDate,1);renderSchedule()});$('#todayShortcut').addEventListener('click',()=>{state.scheduleDate=todayYmd();renderSchedule()});$('#printSchedule').addEventListener('click',()=>window.print());

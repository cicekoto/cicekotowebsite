...
function requestBodyBytes(req) {
  ...
  const declared = rawLength ? (/^\d+$/.test(rawLength) ? Number(rawLength) : Number.POSITIVE_INFINITY) : 0;
  let actual = 0;
  try {
    const body = req?.body;
    if (typeof body === 'string' || Buffer.isBuffer(body)) actual = Buffer.byteLength(body);
    else if (body !== undefined && body !== null) actual = Buffer.byteLength(JSON.stringify(body));
  } catch {
...
function bodyWithinLimit(req, maxBytes) {
  return requestBodyBytes(req) <= Math.max(0, Number(maxBytes) || 0);
}
...
module.exports = { COOKIE_NAME, bodyWithinLimit, clearSessionCookies, createSession, csrfToken, isAdminRequest, readSession, requestBodyBytes, safeEqual, sameOrigin, sessionCookie, verifyCsrf, verifySession };
